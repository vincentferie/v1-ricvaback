...
import { ChargesDto } from './charges.dto';
...
export class ChargesController {
    ...
    constructor(private readonly controllerService: ChargesService) { }

    @Post()
    @UsePipes(ValidationPipe)
    async save(@JwtInfoGetter() payload, @Body() inputDto: ChargesDto) {
    ...
    async update(
        ...
        @Body(ValidationPipe) inputDto: ChargesDto,
    ...
    @Patch('/charge-bl/:primaryKey')
    async updateChargeBl(
        @JwtInfoGetter() payload, 
        @Body(ValidationPipe) inputDto: ChargeBlDto,
        @Param('primaryKey', ParseUUIDPipe) primaryKey){
            inputDto.updated = new Date(Date.now());
            inputDto.updated_by = `${payload.user.nom} ${payload.user.prenoms}`;
            const result = await this.controllerService.updateChargeBl(payload, inputDto, primaryKey);
            return result;
    }
    ...
    @Get('list')
    async listCharge(@JwtInfoGetter() payload) {
        const result = await this.controllerService.listCharge(payload);
        return result;
    }
