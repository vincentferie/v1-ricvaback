...
import { FournisseursModel } from './fournisseurs.model';

@Injectable()
export class FournisseursService {

    constructor(
        @InjectRepository(FournisseursEntity) private readonly entityRepository: Repository<FournisseursEntity>,
    ) { }

    async save(payload: any, input: FournisseursModel): Promise<FournisseursEntity> {
    ...
    async update(payload: any, input: object, primaryKey: string): Promise<FournisseursEntity> {
    ...
    async updateMode(payload: any, primaryKey: string, mode: SoftDelete): Promise<FournisseursEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
    ...
    async find(payload: any, query: object): Promise<FournisseursEntity[]> {
        let exception: any, result: any;

        try {
            const queryFiltering = Object.keys(query).length > 0 ? TypeOrmHttpParamQuery(query) : {
                relations: ['transitaire', 'client', 'exportateur'],
    ...
    async findPaginate(payload: any, query: any): Promise<FournisseursEntity[]>{
    ...
    async findPaginateResearch(payload: any, query: any): Promise<FournisseursEntity[]>{
        let exception: any, result: any, total: number;
        try {        
            [result, total] = await this.entityRepository
                                .createQueryBuilder('four')
                                .leftJoinAndSelect('admin_transitaires', 'trans', 'four.transitaire_id = trans.id')
                                .leftJoinAndSelect('admin_clients', 'client', 'four.client_id = client.id')
                                .leftJoinAndSelect('admin_exportateurs_groupement', 'export', 'four.exportateur_id = export.id')
                                .where('LOWER(CONCAT(four.code," ",four.raison_social," ",four.denomination," ",four.localisation," ",four.activite," ",four.contact)) ILIKE :query', {query: `%${query.query}%`})
                                .andWhere('four.mode = :mode', {mode: SoftDelete.active})
                                .orderBy('four.raison_social', query.order ?? 'DESC')
                                .addOrderBy('four.created', 'DESC')
                                .skip(+query.offset)
                                .take(+query.take)
                                .cache(60000)
                                .getManyAndCount();
    ...
    async findMode(payload: any, mode: SoftDelete): Promise<FournisseursEntity[]> {
        let exception: any, result: any;
    ...
    async findById(payload: any, primaryKey: string): Promise<FournisseursEntity> {
        let exception: any, result: any;
