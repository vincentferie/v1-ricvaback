...
import { OperationDiverseModel } from './operation-diverse.model';
...
@Injectable()
export class OperationDiverseService {

    constructor(
        @InjectRepository(OperationDiverseEntity) private readonly entityRepository: Repository<OperationDiverseEntity>,
        @InjectRepository(EcritureChargesDiversesEntity) private readonly subRepository: Repository<EcritureChargesDiversesEntity>
    ) { }

    async save(payload: any, input: OperationDiverseModel): Promise<OperationDiverseEntity> {
        ...
        try {
            ...
            if (isDefined(result)) {
                // Ecriture comptable
                // Insert line
                const accountWriting = {charge_ops_id: result.id, debit: input.debit, credit: input.credit, created: new Date(Date.now())} as EcritureChargesDiversesModel;
    ...
    async update(payload: any, input: object, primaryKey: string): Promise<OperationDiverseEntity> {
    ...
    async updateMode(payload: any, primaryKey: string, mode: SoftDelete): Promise<OperationDiverseEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
    ...
    async find(payload: any, query: object): Promise<OperationDiverseEntity[]> {
        let exception: any, result: any;
    ...
    async listCharge(payload: any): Promise<OperationDiverseEntity[]>{
        let exception: any, result: any;
        try {
            result = await this.entityRepository
                                .createQueryBuilder("charge")
                                .leftJoin("admin_reglements_charge_diverse", "reglement", "charge.id = denantis.charge_ops_id")
                                .where("reglement.charge_ops_id IS NULL")
    ...
    async findPaginate(payload: any, query: any): Promise<OperationDiverseEntity[]>{
    ...
    async findPaginateResearch(payload: any, query: any): Promise<OperationDiverseEntity[]>{
        let exception: any, result: any, total: number;
        try {               
            [result, total] = await this.entityRepository.findAndCount({
                where: [
                    {
                        num_ordre: Raw((columnAlias) => ` CAST(${columnAlias} AS TEXT) ILIKE :value`, { value: `%${query.query}%`}),
                        mode: SoftDelete.active
                    },
                    {
                        num_facture: ILike(`%${query.query}%`),
                        mode: SoftDelete.active
                    },
                    {
                    ...
                    {
                        montant_ht: Raw((columnAlias) => `CAST(${columnAlias} AS TEXT) ILIKE :value`, { value: `%${query.query}%`}),
                        mode: SoftDelete.active
                    },
                    {
                        date_facture: Raw((columnAlias) => `TO_CHAR(${columnAlias}, 'YYYY-MM-DD HH:MI:SS') ILIKE :value`, { value: `%${query.query}%`}),
                        mode: SoftDelete.active
                    }
                ],
                order: { 
    ...
    async findMode(payload: any, mode: SoftDelete): Promise<OperationDiverseEntity[]> {
        let exception: any, result: any;
    ...
    async findById(payload: any, primaryKey: string): Promise<OperationDiverseEntity> {
        let exception: any, result: any;
