...
import { ReglementChargesModel } from './reglement-charges.model';
...
@Injectable()
export class ReglementChargesService {

    constructor(
        @InjectRepository(ReglementChargesEntity) private readonly entityRepository: Repository<ReglementChargesEntity>,
        @InjectRepository(EcritureChargesEntity) private readonly sub3Repository: Repository<EcritureChargesEntity>,
        ) { }

    async save(payload: any, input: ReglementChargesModel): Promise<ReglementChargesEntity> {
        ...
        try {
            ...
            if (isDefined(result)) {
                ...
                // Ecriture comptable
                const accountWriting = {reglement_charge_id: result.id, debit: input.debit, credit: input.credit, created: new Date(Date.now())} as EcritureChargesModel;
    ...
    async update(payload: any, input: object, primaryKey: string): Promise<ReglementChargesEntity> {
    ...
    async updateMode(payload: any, primaryKey: string, mode: SoftDelete): Promise<ReglementChargesEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
    ...
    async find(payload: any, query: object): Promise<ReglementChargesEntity[]> {
        let exception: any, result: any;
    ...
    async findMode(payload: any, mode: SoftDelete): Promise<ReglementChargesEntity[]> {
        let exception: any, result: any;
    ...
    async findById(payload: any, primaryKey: string): Promise<ReglementChargesEntity> {
        let exception: any, result: any;
