...
import { BilanDataParamsModel } from './bilan-data-params.model';
import { BilanDataParamsDto } from './bilan-data-params.dto';
import { bilanParamsDataSeed } from 'src/seeders/fakers';

@Injectable()
export class BilanDataParamsService {
    constructor(
        ...
        @InjectRepository(BilanDataParamsEntity) private readonly bilanRepository: Repository<BilanDataParamsEntity>,
    ) { }

    async save(payload: any, input: BilanDataParamsModel): Promise<BilanDataParamsEntity> {
        let result: any, exception: any;
        try {
            result = await this.bilanRepository.save(input as BilanDataParamsDto);
            if (isDefined(result)) {
    ...
    async update(payload: any, input: BilanDataParamsModel, primaryKey: string): Promise<BilanDataParamsEntity> {
    ...
    async updateMode(payload: any, primaryKey: string, mode: SoftDelete): Promise<BilanDataParamsEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
    ...
    async find(payload: any, query: object): Promise<BilanDataParamsEntity[]> {
        let exception: any, result: any;
    ...
    async findMode(payload: any, mode: SoftDelete): Promise<BilanDataParamsEntity[]> {
        let exception: any, result: any;
    ...
    async findById(payload: any, primaryKey: string): Promise<BilanDataParamsEntity> {
        let exception: any, result: any;
   ...
   * @function
   */
  create(): Array<Promise<BilanDataParamsEntity>> {
    return bilanParamsDataSeed.map(async (item: BilanDataParamsModel) => {
