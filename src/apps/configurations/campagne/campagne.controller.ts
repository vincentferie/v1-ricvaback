...
export class CampagneController {
    ...
    constructor(private readonly controllerService: CampagneService) { }

    @Post()
    @UsePipes(ValidationPipe)
    async save(@JwtInfoGetter() payload, @Body() inputDto: CampagneDto) {
    ...
    async update(
        ...
        @Body(ValidationPipe) inputDto: CampagneDto,
    ...
    async updateDetails(
        @JwtInfoGetter() payload,
        @Body(ValidationPipe) inputDto: CampagneOutturnDto,
        @Param('primaryKey', ParseUUIDPipe) primaryKey) {
            inputDto.updated = new Date(Date.now());
            inputDto.updated_by = `${payload.user.nom} ${payload.user.prenoms}`;
