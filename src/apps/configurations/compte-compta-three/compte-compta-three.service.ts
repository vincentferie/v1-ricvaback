...
import { CompteComptableThreeModel } from './compte-compta-three.model';
import { CompteComptableThreeDto } from './compte-compta-three.dto';
import { compteLevelThreeSeed } from 'src/seeders/fakers';

@Injectable()
export class CompteComptaThreeService {
    ...
    constructor(
        ...
        @InjectRepository(CompteComptableThreeEntity) private readonly compteRepository: Repository<CompteComptableThreeEntity>,
    ) { }

    async save(payload: any, input: CompteComptableThreeModel): Promise<CompteComptableThreeEntity> {
        let result: any, exception: any;
        try {
            result = await this.compteRepository.save(input as CompteComptableThreeDto);
            if (isDefined(result)) {
    ...
    async update(payload: any, input: CompteComptableThreeModel, primaryKey: string): Promise<CompteComptableThreeEntity> {
    ...
    async updateMode(payload: any, primaryKey: string, mode: SoftDelete): Promise<CompteComptableThreeEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
    ...
    async find(payload: any, query: object): Promise<CompteComptableThreeEntity[]> {
        let exception: any, result: any;
        ...
        try {
            ...
            result = await this.compteRepository.find(queryFiltering);
            if (isDefined(result)) {
                exception = await responseRequest({
                    status: 'foundQuery',
                    data: result,
    ...
    async findMode(payload: any, mode: SoftDelete): Promise<CompteComptableThreeEntity[]> {
        let exception: any, result: any;
    ...
    async findById(payload: any, primaryKey: string): Promise<CompteComptableThreeEntity> {
        let exception: any, result: any;
    ...
    * @function
    */
   create(): Array<Promise<CompteComptableThreeEntity>> {
    return compteLevelThreeSeed.map(async (item: CompteComptableThreeModel) => {
        return await this.compteRepository
        .findOne({ numero: item.numero, libelle: item.libelle })
        .then(async dbresult => {
