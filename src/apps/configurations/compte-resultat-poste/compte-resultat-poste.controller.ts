...
@ApiTags('compte-resultat-poste')
@Controller('compte-resultat-poste')
@UseGuards(AuthGuard('jwt'))
@Roles(Role.Manager, Role.Administrator, Role.Accounting)
export class CompteResultatPosteController {

    constructor(private readonly compteComptaService: CompteResultatPosteService) { }

    @Post()
    @UsePipes(ValidationPipe)
    async save(@JwtInfoGetter() payload, @Body() compteDto: CompteResultatPosteParamsDto) {
    ...
    async update(
        ...
        @Body(ValidationPipe) compteDto: CompteResultatPosteParamsDto,
