...
import { ExportateursGroupementModel } from './exportateurs-groupement.model';

@Injectable()
export class ExportateursGroupementService {

    constructor(
        @InjectRepository(ExportateursGroupementEntity) private readonly entityRepository: Repository<ExportateursGroupementEntity>,
    ) { }

    async save(payload: any, input: ExportateursGroupementModel): Promise<ExportateursGroupementEntity> {
    ...
    async update(payload: any, input: object, primaryKey: string): Promise<ExportateursGroupementEntity> {
    ...
    async updateMode(payload: any, primaryKey: string, mode: SoftDelete): Promise<ExportateursGroupementEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
    ...
    async list(payload: any, primaryKey: string): Promise<ExportateursGroupementEntity[]>{
        let exception: any, result: any;
        try {
            result = await this.entityRepository.find({
                relations: ['groupement', 'exportateur'],
                where: [{
                    groupement_id: primaryKey, 
    ...
    async find(payload: any, query: object): Promise<ExportateursGroupementEntity[]> {
        let exception: any, result: any;

        try {
            query = Object.keys(query).length > 0 ? TypeOrmHttpParamQuery(query) : { relations: ['groupement', 'exportateur'], where: [{mode: SoftDelete.active}]};
    ...
    async findMode(payload: any, mode: SoftDelete): Promise<ExportateursGroupementEntity[]> {
        let exception: any, result: any;

        try {
            result = await this.entityRepository.find({ relations: ['groupement', 'exportateur'], where: [{mode: mode}]});
    ...
    async findById(payload: any, primaryKey: string): Promise<ExportateursGroupementEntity> {
        let exception: any, result: any;

        try {
            result = await this.entityRepository.findOne({
                relations: ['groupement', 'exportateur'], where: [{id: primaryKey, mode: SoftDelete.active}]
                  });
