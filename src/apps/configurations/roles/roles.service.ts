...
import { roleSeed } from 'src/seeders/fakers';
@Injectable()
export class RolesService {

    constructor(
        //  Ci apres S'utilise avec un fichier repository. Mais ne peut plus creer de methode save update...
        // @InjectRepository(ProfilRepository) private roleRepository: ProfilRepository, 
        // Ci apres S'utilise sans fichier repository
        @InjectRepository(RoleEntity) private readonly roleRepository: Repository<RoleEntity>,
    ) { }

    async save(payload: any, input: RoleModel): Promise<RoleEntity> {
        let result: any, exception: any;
        try {
            result = await this.roleRepository.save(input);
            if (isDefined(result)) {
    ...
    async update(payload: any, input: object, primaryKey: string): Promise<RoleEntity> {
        ...
        try {
            ...
            result = await this.roleRepository.save({ ...response.data, ...input });
    ...
    async updateMode(payload: any, primaryKey: string, mode: SoftDelete): Promise<RoleEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
        ...
        try {
            ...
            result = await this.roleRepository.save(response.data);
    ...
    async delete(payload: any, primaryKey: string): Promise<void> {
        ...
        try {
            result = await this.roleRepository.delete(primaryKey);
    ...
    async find(payload: any, query: object): Promise<RoleEntity[]> {
        let exception: any, result: any;

        try {
            result = await this.roleRepository.find(TypeOrmHttpParamQuery(query));
            if (isDefined(result)) {
                exception = await responseRequest({
                    status: 'foundQuery',
                    data: result,
    ...
    async findMode(payload: any, mode: SoftDelete): Promise<RoleEntity[]> {
        let exception: any, result: any;

        try {
            result = await this.roleRepository.find({ mode: mode });
    ...
    async findById(payload: any, primaryKey: string): Promise<RoleEntity> {
        let exception: any, result: any;

        try {
            result = await this.roleRepository.findOne({ id: primaryKey, mode: SoftDelete.active  });
    ...
    async checkById(primaryKey: string): Promise<any> {
        ...
        try {
            result = await this.roleRepository.findOne({ id: primaryKey });
            if (isDefined(result)) {
                exception = {
                    status: 202,
                    data: result,
   ...
   create(): Array<Promise<RoleEntity>> {
    return roleSeed.map(async (item: RoleModel) => {
