...
export class SitesService {
    ...
    constructor(
        @InjectRepository(SiteEntity) private readonly entityRepository: Repository<SiteEntity>,
    ) { }

    async save(payload: any, input: SiteModel): Promise<SiteEntity> {
    ...
    async update(payload: any, input: object, primaryKey: string): Promise<SiteEntity> {
    ...
    async updateMode(payload: any, primaryKey: string, mode: SoftDelete): Promise<SiteEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
    ...
    async find(payload: any, query: object): Promise<SiteEntity[]> {
        let exception: any, result: any;
    ...
    async findPaginate(payload: any, query: any): Promise<SiteEntity[]>{
    ...
    async findPaginateResearch(payload: any, query: any): Promise<SiteEntity[]>{
        let exception: any, result: any, total: number;
        try {
            ...
            [result, total] = await this.entityRepository.findAndCount({
                ...
                where:[
                    ...
                    {
                        superficie: Raw((columnAlias) => `CAST(${columnAlias} AS TEXT) ILIKE :value`, { value: `%${query.query}%`}),
                        mode: SoftDelete.active
                    },
                ],
                order: { 
    ...
    async findMode(payload: any, mode: SoftDelete): Promise<SiteEntity[]> {
        let exception: any, result: any;

        try {
            result = await this.entityRepository.find({ relations:['ville'], where: [{mode: mode}] });
    ...
    async findById(payload: any, primaryKey: string): Promise<SiteEntity> {
        let exception: any, result: any;

        try {
            result = await this.entityRepository.findOne({ relations:['ville'], where: [{id: primaryKey}] });
            if (isDefined(result)) {
                exception = await responseRequest({
                    status: 'foundQuery',
                    data: result,
