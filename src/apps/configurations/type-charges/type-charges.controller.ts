...
import { TypeChargesDto } from './type-charges.dto';
...
@ApiTags('type-charges')
@Controller('type-charges')
@UseGuards(AuthGuard('jwt'))
@Roles(Role.Manager, Role.Administrator)
export class TypeChargesController {

    constructor(private readonly controllerService: TypeChargesService) { }

    @Post()
    @UsePipes(ValidationPipe)
    async save(@JwtInfoGetter() payload, @Body() inputDto: TypeChargesDto) {
    ...
    async update(
        ...
        @Body(ValidationPipe) inputDto: TypeChargesDto,
