...
import { BanquesSpecModel } from './banques-spec.model';
import { BanquesSpecEntity } from './banques-spec.entity';
@Injectable()
export class BanquesSpecService {

    constructor(
        @InjectRepository(BanquesSpecEntity) private readonly entityRepository: Repository<BanquesSpecEntity>,
    ) { }

    async save(payload:any, input: BanquesSpecModel): Promise<BanquesSpecEntity> {
    ...
    async update(payload:any, input: object, primaryKey: string): Promise<BanquesSpecEntity> {
    ...
    async updateMode(payload:any, primaryKey: string, mode: SoftDelete): Promise<BanquesSpecEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
    ...
    async find(payload:any, query: object): Promise<BanquesSpecEntity[]> {
        let exception: any, result: any;

        try {
            result = await this.entityRepository.find(Object.keys(query).length > 0 ? TypeOrmHttpParamQuery(query): {relations:['banques'], where: [{mode: SoftDelete.active}] });
            if (isDefined(result)) {
                exception = await responseRequest({
                    status: 'foundQuery',
                    data: result,
    ...
    async findMode(payload:any, mode: SoftDelete): Promise<BanquesSpecEntity[]> {
        let exception: any, result: any;

        try {
            result = await this.entityRepository.find({ relations:['banques'], where: [{mode: mode}] });
    ...
    async findById(payload:any, primaryKey: string): Promise<BanquesSpecEntity> {
        let exception: any, result: any;

        try {
            result = await this.entityRepository.findOne({  relations:['banques'], where: [{id: primaryKey, mode: SoftDelete.active}]});
