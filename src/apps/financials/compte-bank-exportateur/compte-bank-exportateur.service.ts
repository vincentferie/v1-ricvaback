...
import { CompteBankExportateurModel } from './compte-bank-exportateur.model';
import { StateTirage } from 'src/helpers/enums/state.enum';
@Injectable()
export class CompteBankExportateurService {

    constructor(
        @InjectRepository(CompteBankExportateurEntity) private readonly entityRepository: Repository<CompteBankExportateurEntity>,
    ) { }

    async save(payload:any, input: CompteBankExportateurModel): Promise<CompteBankExportateurEntity> {
    ...
    async update(payload:any, input: object, primaryKey: string): Promise<CompteBankExportateurEntity> {
    ...
    async updateMode(payload:any, primaryKey: string, mode: SoftDelete): Promise<CompteBankExportateurEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
    ...
    async find(payload:any, query: object): Promise<CompteBankExportateurEntity[]> {
        let exception: any, result: any;

        try {
            const queryFiltering = Object.keys(query).length > 0 ? TypeOrmHttpParamQuery(query) : {
                relations: ['finGroupExport', 'banque', 'campagne'],
    ...
    async findPaginate(payload: any, query: any): Promise<CompteBankExportateurEntity[]>{
    ...
    async findPaginateResearch(payload: any, query: any): Promise<CompteBankExportateurEntity[]>{
        let exception: any, result: any, total: number;
        try {        
            [result, total] = await this.entityRepository.findAndCount({
                                    relations: ['finGroupExport', 'banque', 'campagne'],
                                    where: [
                                        ...
                                        {
                                            date_tirage: Raw((columnAlias) => `TO_CHAR(${columnAlias}, 'YYYY-MM-DD HH:MI:SS') ILIKE :value`, { value: `%${query.query}%`}),
                                            mode: SoftDelete.active
                                        },
                                    ],
                                    order: { 
    ...
    async findMode(payload:any, mode: SoftDelete): Promise<CompteBankExportateurEntity[]> {
        let exception: any, result: any;
    ...
    async findById(payload:any, primaryKey: string): Promise<CompteBankExportateurEntity> {
        let exception: any, result: any;
    ...
    async tirageType(): Promise<any>{
        const exception = await responseRequest({
            status: 'foundQuery',
            data: {
                'Découvert' : StateTirage.decouvert,
                'Nantissement' : StateTirage.nantissement,
                'Rapatriement' : StateTirage.rapatriement,
                'Subvention' : StateTirage.subvention,
                'Autres' : StateTirage.autres,
            },
            params: 1
        });
        throw new HttpException(exception[0], exception[1]);
