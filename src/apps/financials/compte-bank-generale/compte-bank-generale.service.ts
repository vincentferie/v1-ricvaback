...
import { CompteBankGeneraleModel } from './compte-bank-generale.model';
@Injectable()
export class CompteBankGeneraleService {

    constructor(
        @InjectRepository(CompteBankGroupementEntity) private readonly entityRepository: Repository<CompteBankGroupementEntity>,
    ) { }

    async save(payload:any, input: CompteBankGeneraleModel): Promise<CompteBankGroupementEntity> {
    ...
    async update(payload:any, input: object, primaryKey: string): Promise<CompteBankGroupementEntity> {
    ...
    async updateMode(payload:any, primaryKey: string, mode: SoftDelete): Promise<CompteBankGroupementEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
    ...
    async find(payload:any, query: object): Promise<CompteBankGroupementEntity[]> {
        let exception: any, result: any;

        try {
            const queryFiltering = Object.keys(query).length > 0 ? TypeOrmHttpParamQuery(query) : {
                relations: ['groupements', 'banque', 'campagne'],
    ...
    async findPaginate(payload: any, query: any): Promise<CompteBankGroupementEntity[]>{
    ...
    async findPaginateResearch(payload: any, query: any): Promise<CompteBankGroupementEntity[]>{
        let exception: any, result: any, total: number;
        try {        
            [result, total] = await this.entityRepository.findAndCount({
                                    relations: ['groupements', 'banque', 'campagne'],
    ...
    async findMode(payload:any, mode: SoftDelete): Promise<CompteBankGroupementEntity[]> {
        let exception: any, result: any;
    ...
    async findById(payload:any, primaryKey: string): Promise<CompteBankGroupementEntity> {
        let exception: any, result: any;
