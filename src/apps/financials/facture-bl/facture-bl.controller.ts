...
import { FactureBlDto } from './facture-bl.dto';
...
export const destination = './uploads/factures';
@ApiTags('facture-bl')
@Controller('facture-bl')
@UseGuards(AuthGuard('jwt'))
@Roles(Role.Manager, Role.Financial)
export class FactureBlController {
       
    constructor(private readonly controllerService: FactureBlService) { }

    @Post()
    @UsePipes(ValidationPipe)
    async save(
        @JwtInfoGetter() payload, 
        @Body() inputDto: FactureBlDto,
        ) {
    ...
    async update(
        @JwtInfoGetter() payload, 
        @Body(ValidationPipe) inputDto: FactureBlDto,
    ...
    @Get('list')
    async listFacturation(@JwtInfoGetter() payload) {
        const result = await this.controllerService.listFacturation(payload);
        return result;
    }
