...
import { ReglementsFactureBlDto } from './reglement-factures-bl.dto';

@Injectable()
export class ReglementFacturesBlService {
    
    constructor(
        @InjectRepository(ReglementsFactureBlEntity) private readonly entityRepository: Repository<ReglementsFactureBlEntity>,
    ) { }

    async save(payload: any, input: ReglementsFactureBlDto): Promise<ReglementsFactureBlEntity> {
    ...
    async update(payload: any, input: object, primaryKey: string): Promise<ReglementsFactureBlEntity> {
    ...
    async updateMode(payload: any, primaryKey: string, mode: SoftDelete): Promise<ReglementsFactureBlEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
    ...
    async find(payload: any, query: object): Promise<ReglementsFactureBlEntity[]> {
        let exception: any, result: any;

        try {
            const queryFiltering = Object.keys(query).length > 0 ? TypeOrmHttpParamQuery(query) : {
                relations: ['facture', 'banque', 'campagne'],
    ...
    async findMode(payload: any, mode: SoftDelete): Promise<ReglementsFactureBlEntity[]> {
        let exception: any, result: any;
    ...
    async findById(payload: any, primaryKey: string): Promise<ReglementsFactureBlEntity> {
        let exception: any, result: any;
