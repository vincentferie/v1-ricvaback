...
import { AnalysesModel } from './analyse.model';
@Catch()
@Injectable()
export class AnalysesService {

    constructor(
        @InjectRepository(AnalysesEntity) private readonly entityRepository: Repository<AnalysesEntity>,
    ) { }

    async save(input: AnalysesModel): Promise<AnalysesEntity> {
    ...
    async update(input: object, primaryKey: string): Promise<AnalysesEntity> {
    ...
    async updateMode(payload:any, primaryKey: string, mode: SoftDelete): Promise<AnalysesEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
    ...
    async find(payload:any, query: object): Promise<AnalysesEntity[]> {
        let exception: any, result: any;
    ...
    async findPaginate(payload: any, query: any): Promise<AnalysesEntity[]>{
    ...
    async findPaginateResearch(payload: any, query: any): Promise<AnalysesEntity[]>{
        let exception: any, result: any, total: number;
        try {
            [result, total] = await this.entityRepository
                                .createQueryBuilder("analyse")
                                .leftJoinAndSelect("analyse.lots", "lot", "lot.id = analyse.lot_id")
                                .where('LOWER(CONCAT(lot.numero_lot," ",lot.numero_ticket_pese," ",th," ",grainage," ",out_turn)) ILIKE :query', {query: `%${query.query}%`})
                                .andWhere('analyse.mode = :mode', {mode: SoftDelete.active})
                                .orderBy('out_turn', query.order ?? 'ASC')
                                .addOrderBy('analyse.created', 'DESC')
                                .skip(+query.offset)
                                .take(+query.take)
                                .cache(60000)
                                .getManyAndCount();
    ...
    async findMode(payload:any, mode: SoftDelete): Promise<AnalysesEntity[]> {
        let exception: any, result: any;
    ...
    async findById(payload:any, primaryKey: string): Promise<AnalysesEntity> {
        let exception: any, result: any;
