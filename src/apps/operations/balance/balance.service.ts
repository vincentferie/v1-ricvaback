...
export class BalanceService {
    ...
    constructor(
        @InjectRepository(BalanceEntity) private readonly entityRepository: Repository<BalanceEntity>,
    ) { }

    async save(input: BalanceModel): Promise<BalanceEntity> {
    ...
    async update(input: object, primaryKey: string): Promise<BalanceEntity> {
    ...
    async updateMode(payload: any, primaryKey: string, mode: SoftDelete): Promise<BalanceEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
    ...
    async find(payload: any, query: object): Promise<BalanceEntity[]> {
        let exception: any, result: any;
    ...
    async findPaginate(payload: any, query: any): Promise<BalanceEntity[]>{
    ...
    async findPaginateResearch(payload: any, query: any): Promise<BalanceEntity[]>{
        let exception: any, result: any, total: number;
        try {
            [result, total] = await this.entityRepository
                                .createQueryBuilder("balance")
                                .leftJoinAndSelect("balance.lots", "lot", "lot.id = balance.lot_id")
                                .leftJoinAndSelect("balance.entrepot", "entrepot")
                                .leftJoinAndSelect("balance.campagne", "campagne")
                                .where('LOWER(CONCAT(lot.numero_lot," ",lot.numero_ticket_pese," ",balance.nbre_sacs," ",balance.date)) ILIKE :query', {query: `%${query.query}%`})
                                .orWhere('balance.lot_id IS NULL')
                                .orWhere('balance.lot_id IS NOT NULL')
                                .andWhere('balance.mode = :mode', {mode: SoftDelete.active})
                                .orderBy('balance.date', query.order ?? 'DESC')
                                .addOrderBy('balance.created', 'DESC')
                                .skip(+query.offset)
                                .take(+query.take)
                                .cache(60000)
                                .getManyAndCount();
    ...
    async findMode(payload: any, mode: SoftDelete): Promise<BalanceEntity[]> {
        let exception: any, result: any;
    ...
    async findById(payload: any, primaryKey: string): Promise<BalanceEntity> {
        let exception: any, result: any;
