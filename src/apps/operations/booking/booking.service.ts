...
export class BookingService {
    ...
    constructor(
        @InjectRepository(BookingEntity) private readonly entityRepository: Repository<BookingEntity>,
    ) { }

    async save(payload: any, input: BookingModel): Promise<BookingEntity> {
    ...
    async update(payload: any, input: object, primaryKey: string): Promise<BookingEntity> {
    ...
    async closeBooking(payload: any, primaryKey: string): Promise<BookingEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
        ...
        try {
            response.data.state = StateBooking.terminer;
    ...
    async updateMode(payload: any, primaryKey: string, mode: SoftDelete): Promise<BookingEntity> {
        let exception: any, result: any;
        const response = await this.checkById(primaryKey);
    ...
    async find(payload: any, query: object): Promise<BookingEntity[]> {
        let exception: any, result: any;
    ...
    async list(payload: any): Promise<BookingEntity[]>{
        let exception: any, result: any;
        try {
            result = await this.entityRepository
                                .createQueryBuilder("booking")
                                //.leftJoin("admin_conteneur", "conteneur", "booking.id = conteneur.booking_id")
                                //.where("conteneur.booking_id IS NULL")
                                //.orWhere("COUNT(conteneur.booking_id) >= 1")
                                .where("booking.state = :state::admin_booking_state_enum", {state: StateBooking.encours})
                                .andWhere("booking.mode = :mode",{mode : SoftDelete.active})
                                .getMany();
    ...
    async findPaginate(payload: any, query: any): Promise<BookingEntity[]>{
    ...
    async findPaginateResearch(payload: any, query: any): Promise<BookingEntity[]>{
        let exception: any, result: any, total: number;
        try {
            [result, total] = await this.entityRepository.findAndCount({
                where: [
                    {
                        numero_reel: ILike(`%${query.query}%`),
                        mode: SoftDelete.active
                    },
                    {
                        numero_change: ILike(`%${query.query}%`),
                        mode: SoftDelete.active
                    }
                ],
                order: { 
    ...
    async findMode(payload: any, mode: SoftDelete): Promise<BookingEntity[]> {
        let exception: any, result: any;
    ...
    async findById(payload: any, primaryKey: string): Promise<BookingEntity> {
        let exception: any, result: any;
