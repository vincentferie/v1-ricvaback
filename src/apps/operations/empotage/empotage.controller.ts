...
import { EmpotagesDto } from './empotage.dto';
...
@ApiTags('empotages')
@Controller('empotages')
@UseGuards(AuthGuard('jwt'))
@Roles(Role.Manager, Role.Operator)
export class EmpotagesController {
    
    constructor(private readonly controllerService: EmpotagesService) { }

    @Post()
    @UsePipes(ValidationPipe)
    async save(@JwtInfoGetter() payload, @Body() inputDto: EmpotagesDto) {
    ...
    async update(
        ...
        @Body(ValidationPipe) inputDto: EmpotagesDto,
    ...
    async updateDetails(
        @JwtInfoGetter() payload, 
        @Body(ValidationPipe) inputDto: LotEmpoteDto,
        @Param('primaryKey', ParseUUIDPipe) primaryKey) {
            inputDto.updated = new Date(Date.now());
            inputDto.updated_by = `${payload.user.nom} ${payload.user.prenoms}`;
    ...
    @Get('try')
    async findMode(@JwtInfoGetter() payload, @Param('primaryKey', ParseUUIDPipe) primaryKey, @Query('mode', SoftDeleteValidationPipe) mode: SoftDelete) {
        const result = await this.controllerService.findMode(payload, primaryKey, mode);
        return result;
    }
    ...
    @Get('site/:primaryKey')
    async findSite(@JwtInfoGetter() payload, @Param('primaryKey', ParseUUIDPipe) primaryKey) {
        const result = await this.controllerService.find(payload, {where: [{site_id: primaryKey}]});
        return result;
    }
